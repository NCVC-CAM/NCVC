...
*/
	if ( CMDIChildWnd::OnCmdMsg(nID, nCode, pExtra, pHandlerInfo) )
		return TRUE;
	CNCDoc* pDoc = (CNCDoc *)GetActiveDocument();
	if ( !pDoc )
		return FALSE;
...
void CNCChild::OnUpdateStatusLineNo(int nPos, int nSize, const CNCblock* pBlock)
{
	CString	strInfo;
	...
	if ( pBlock ) {
		...
		if ( nError > 0 ) {
			...
			CNCdata*	pData = pBlock->GetBlockToNCdata();
			if ( pData ) {
...
LRESULT CNCFrameSplit::OnUserInitialUpdate(WPARAM wParam, LPARAM)
{
	...
	nInfo = min(AfxGetNCVCMainWnd()->GetNCTextHeight() * 10, rc.Height()/3);
	if ( (BOOL)wParam ) {

...
void CNCdata::DrawBottomFace(optional<double>&) const
{
}
...
void CNCline::DrawBottomFace(optional<double>& dz) const
{
	if ( m_nc.nGcode != 1 )
		return;
...
//		::glBegin(GL_QUAD_STRIP);
		...
		for ( i=0; i<=ARCCOUNT/2; i++ ) {
			::glVertex3d(pt2[i].x, pt2[i].y, sz);
			::glVertex3d(pt1[i].x, pt1[i].y, ez);
		}
		::glEnd();
	}
	...
	::glNormal3d(0.0, 0.0, 1.0);
	::glVertex3d(m_ptValE.x, m_ptValE.y, ez);
	for ( i=0; i<=ARCCOUNT/2; i++ )
		::glVertex3d(pt2[i].x, pt2[i].y, ez);
	for ( i=0; i< ARCCOUNT/2; i++ )
		::glVertex3d(pt3[i].x, pt3[i].y, ez);
	::glVertex3d(pt2[0].x, pt2[0].y, ez);
	::glEnd();
}
...
void CNCcycle::DrawBottomFace(optional<double>& dz) const
{
	int		i, j;
	CPointD	pt[ARCCOUNT+1];
	double	cz;
	...
	for ( i=0; i<m_nDrawCnt; i++ ) {
		// 座標計算
		SetEndmillPath(m_Cycle3D[i].ptC, pt);
		cz = dz ? *dz : m_Cycle3D[i].ptC.z;
		// 底面描画
		::glBegin(GL_TRIANGLE_FAN);
		::glNormal3d(0.0, 0.0, 1.0);
		::glVertex3d(m_Cycle3D[i].ptC.x, m_Cycle3D[i].ptC.y, cz);	// 中心
		for	( j=0; j<=ARCCOUNT; j++ )
			::glVertex3d(pt[j].x, pt[j].y, cz);
		::glEnd();
	}
}
...
void CNCcircle::DrawBottomFace(optional<double>& dz) const
{
	size_t	i;
	vector<CPointD>	vt1, vt2;
	...
	::glBegin(GL_TRIANGLE_FAN);
	::glNormal3d(0.0, 0.0, 1.0);
	::glVertex3d(m_ptValS.x, m_ptValS.y, cz);
	for ( i=0; i<=ARCCOUNT/2; i++ )
		::glVertex3d(pt3[i].x, pt3[i].y, cz);
	::glEnd();
	// 底面
//	::glBegin(GL_QUAD_STRIP);
	::glBegin(GL_TRIANGLE_STRIP);
	::glNormal3d(0.0, 0.0, 1.0);
	for ( i=0, z=cz; i<vt1.size() && i<vt2.size(); i++, z+=s ) {
		pt1 = vt1[i];
		pt2 = vt2[i];

...
BEGIN_MESSAGE_MAP(CNCViewXY, CView)
	...
	//}}AFX_MSG_MAP
	// ﾍﾟｰｼﾞ切替ｲﾍﾞﾝﾄ
	ON_MESSAGE (WM_USERACTIVATEPAGE, &CNCViewXY::OnUserActivatePage)
	// 各ﾋﾞｭｰへのﾌｨｯﾄﾒｯｾｰｼﾞ
	ON_MESSAGE (WM_USERVIEWFITMSG, &CNCViewXY::OnUserViewFitMsg)
	// ﾒﾆｭｰｺﾏﾝﾄﾞ
	ON_UPDATE_COMMAND_UI(ID_EDIT_COPY, &CNCViewXY::OnUpdateEditCopy)
	ON_UPDATE_COMMAND_UI_RANGE(ID_VIEW_UP,  ID_VIEW_RT,  &CNCViewXY::OnUpdateMoveKey)
	ON_UPDATE_COMMAND_UI_RANGE(ID_VIEW_RUP, ID_VIEW_RRT, &CNCViewXY::OnUpdateRoundKey)
	ON_COMMAND(ID_EDIT_COPY, &CNCViewXY::OnEditCopy)
	ON_COMMAND_RANGE(ID_VIEW_UP,     ID_VIEW_RT,    &CNCViewXY::OnMoveKey)
	ON_COMMAND_RANGE(ID_VIEW_BEFORE, ID_VIEW_LENSN, &CNCViewXY::OnLensKey)
END_MESSAGE_MAP()
...
{
#ifdef _DEBUG_CMDMSG
	g_dbg.printf("CNCViewXY::OnCmdMsg()");
#endif
...
void CNCViewXY::OnDraw(CDC* pDC)
{
	...
	for ( i=GetDocument()->GetTraceStart(); i<nDraw; i++ ) {
		...
		if ( pData->GetGtype() == G_TYPE )
			(pData->*m_pfnDrawProc)(pDC, FALSE);	// pData->DrawXY(pDC, FALSE);
	}
...
void CNCViewXY::OnUpdateMoveKey(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(TRUE);
}

void CNCViewXY::OnUpdateRoundKey(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(FALSE);
}
...
{
#ifdef _DEBUG_FILEOPEN
	g_dbg.printf("CNCViewXY::OnCreate() Start");
#endif

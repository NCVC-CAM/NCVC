...
static	function<double (const string&)>	g_pfnFeedAnalyze;
// 工具径解析
...
static	function<int (const string&, CNCblock*)>	g_pfnSearchMacro,
...
struct CGcodeParser : qi::grammar<Iterator, Skipper, string()>
{
	...
	CGcodeParser() : CGcodeParser::base_type(rule) {
		using qi::char_;
		using qi::double_;
		rule = qi::raw[ (qi::upper >> double_) |
				(g_chCm >> (char_('R')|'C') >> double_) ];
	}
};
...
{
	qi::rule<Iterator, qi::space_type>		ruleTop, rrMM,
		ruleOrder, ruleValue,
	...
	CCommentParser() : CCommentParser::base_type(ruleTop) {
		...
		// Endmill
		rs1 = no_case[ascii::string(ENDMILL_S)|DRILL_S|TAP_S|REAMER_S] >> g_chEq;
		rr1 = r11|r12;
		r11 = double_[_SetEndmill()] >> rrMM >>
						-(g_chCm >> qi::digit[_SetEndmillType()]);
		r12 = (char_('R')|'r') >> double_[_SetBallEndmill()] >>	// ﾎﾞｰﾙｴﾝﾄﾞﾐﾙ表記
						rrMM;
		// WorkRect
		rs2 = no_case[WORKRECT_S] >> g_chEq;
		rr2 = double_[_SetWorkRect()] % g_chCm;
		// WorkCylinder
		rs3 = no_case[WORKCYLINDER_S] >> g_chEq;
		rr3 = double_[_SetWorkCylinder()] % g_chCm;
		// ViewMode
		rs4 = no_case[LATHEVIEW_S] >> g_chEq;
		rr4 = double_[_SetLatheView()] % g_chCm;
		rs5 = no_case[WIREVIEW_S]  >> g_chEq;
		rr5 = double_[_SetWireView()];
		// ToolPos
		rs6 = no_case[TOOLPOS_S] >> g_chEq;
		rr6 = -double_[_ToolPosX()] >>
				-(g_chCm >> -double_[_ToolPosY()] >>
				-(g_chCm >> -double_[_ToolPosZ()]));
		// "mm"
...
#define confix_p(S, E)	repository::confix(S, E)[*(qi::char_ - E)]
						// (qi::char_('(') >> *(qi::char_ - ')') >> ')')
...
int NC_GSeparater(int nLine, CNCdata*& pDataResult)
{
	...
	// 結果、ｽﾋﾟｰﾄﾞUP(??)
	static	SkipperType	skip_p = ascii::space | confix_p('(', ')');
...
int NC_SearchSubProgram(int* pRepeat)
{
	...
	if ( g_ncArgv.nc.dwValFlags & NCD_L ) {
		*pRepeat = (int)g_ncArgv.nc.dValue[NCA_L];
		...
		n = strBuf.length();
		if ( n > 4 ) {
			*pRepeat = lexical_cast<int>(strBuf.substr(0, n-4));
			nProg    = lexical_cast<int>(strBuf.substr(n-4, 4));
		}
		else {
			*pRepeat = 1;
			nProg    = (int)g_ncArgv.nc.dValue[NCA_P];

...
class CDXFMake
{
	...
	static	CString	GROUPCODE(int nCode) {
		// ｸﾞﾙｰﾌﾟｺｰﾄﾞを3桁右詰にする
		CString	strResult;
	...
	static	CString		MakeSection(enSECNAME enType) {
		...
		// ﾊﾟﾀｰﾝ組み立て
		return GROUPCODE(g_nGroupCode[GROUP0])+
					g_szSection[SEC_SECTION]+gg_szReturn+
					GROUPCODE(g_nGroupCode[GROUP2])+
	...
	static	CString		MakeEndSec(void) {
		...
		extern	LPCTSTR	g_szSection[];
		// ﾊﾟﾀｰﾝ組み立て
		return GROUPCODE(g_nGroupCode[GROUP0])+g_szSection[SEC_ENDSEC]+gg_szReturn;
	}
	// 値生成
	static	CString		MakeValue(int nCode, double dVal) {
		extern	LPCTSTR	gg_szReturn;
		CString	strResult, strFormat;
		strFormat.Format(IDS_MAKENCD_FORMAT, dVal);
		strResult = GROUPCODE(nCode) + strFormat + gg_szReturn;
		return strResult;
	}
	static	CString		MakeValue(DWORD dwFlags, double dVal[]) {
		extern	const	DWORD	g_dwValSet[];
		extern	int		g_nValueGroupCode[];
		CString	strResult;
		for ( int i=0; i<DXFMAXVALUESIZE; i++ ) {
			if ( dwFlags & g_dwValSet[i] )
				strResult += MakeValue(g_nValueGroupCode[i], dVal[i]);
		}
		return strResult;
	}
	static	CString		MakeValue(int nCode, int nVal) {
		CString	strResult, strFormat;
		strFormat.Format(IDS_MAKEDXF_INTVALUE, nVal);
		strResult = GROUPCODE(nCode) + strFormat;
		return strResult;
	}
	...
	static	CString		MakeDxfInfo(int nType, int nLayer) {
		...
		strResult = GROUPCODE(g_nGroupCode[GROUP0])+
						g_szEntitiesKey[nType]+gg_szReturn+
						GROUPCODE(g_nGroupCode[GROUP8])+
						ms_pMakeOpt->GetStr(nLayer)+gg_szReturn;
		if ( nType != TYPE_POINT )
			strResult += GROUPCODE(g_nGroupCode[GROUP6])+
				g_penStyle[ms_pMakeOpt->GetNum(nLType)].lpszDXFname+gg_szReturn;
		return strResult+MakeValue(62, ms_pMakeOpt->GetNum(nLCol)+1);
	}

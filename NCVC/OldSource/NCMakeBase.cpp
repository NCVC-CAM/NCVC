...
//		(*ms_pfnGetValString)(NCAX, pt.x, FALSE) + (*ms_pfnGetValString)(NCAY, pt.y, FALSE) );
		GetValString(NCAX, pt.x, FALSE) + GetValString(NCAY, pt.y, FALSE) );
}
...
//					(*ms_pfnGetValString)(NCAY, pData->GetEndMakePoint().y, FALSE);
					GetValString(NCAX, pData->GetEndMakePoint().x, FALSE) +
					GetValString(NCAY, pData->GetEndMakePoint().y, FALSE);
			// 最初だけ送り速度追加
			if ( bFeed && !strGcode.IsEmpty() ) {
				strGcode += GetFeedString(dFeed);
				bFeed = FALSE;
...
//		strResult += (*ms_pfnGetValString)(n, dValue[n], FALSE);
		strResult += GetValString(n, dValue[n], FALSE);
	}
	return strResult;
}
...
//		(*ms_pfnGetValString)(NCA_R, r,    FALSE) );
		GetValString(NCAX, pt.x, FALSE) +
		GetValString(NCAY, pt.y, FALSE) +
		GetValString(NCA_R, r,    FALSE) );
}
...
//		(*ms_pfnGetValString)(NCAJ, ptij.y, FALSE) );
		GetValString(NCAX, pt.x,   FALSE) +
		GetValString(NCAY, pt.y,   FALSE) +
		GetValString(NCAI, ptij.x, FALSE) +
		GetValString(NCAJ, ptij.y, FALSE) );
}
...
//		(*ms_pfnGetValString)(NCA_Z, pt.z, FALSE) );	// ﾍﾘｶﾙはMILLのみ
		GetValString(NCAX,  pt.x, FALSE) +
		GetValString(NCAY,  pt.y, FALSE) +
		GetValString(NCA_Z, pt.z, FALSE) );	// ﾍﾘｶﾙはMILLのみ
}
...
//						(*ms_pfnGetValString)(NCAI, pCircle->GetIJK(NCA_I), FALSE) ); 
						GetValString(NCAJ, pCircle->GetIJK(NCA_J), FALSE) :
						GetValString(NCAI, pCircle->GetIJK(NCA_I), FALSE) ); 
...
//	CString	strGcode, strBuf( (*ms_pfnGetValString)(NCA_R, r, FALSE) );
	CString	strGcode, strBuf( GetValString(NCA_R, r, FALSE) );
	if ( !strBuf.IsEmpty() ) {
		// 計算順序の関係で１行にできない
...
//					(*ms_pfnGetValString)(NCA_Z, dHelical, FALSE) +
					GetValString(NCA_Z, dHelical, FALSE) +
					strBuf + GetFeedString(dFeed) + ms_strEOB;
	return strGcode;
}
...
//	CString	strBuf1( (*ms_pfnGetValString)(NCAI, ij.x, FALSE) + (*ms_pfnGetValString)(NCAJ, ij.y, FALSE) );
	CString	strBuf1( GetValString(NCAI, ij.x, FALSE) + GetValString(NCAJ, ij.y, FALSE) );
	ij *= -1.0;		// ij = -ij;
//	CString	strBuf2( (*ms_pfnGetValString)(NCAI, ij.x, FALSE) + (*ms_pfnGetValString)(NCAJ, ij.y, FALSE) );
	CString	strBuf2( GetValString(NCAI, ij.x, FALSE) + GetValString(NCAJ, ij.y, FALSE) );
	if ( !strBuf1.IsEmpty() && !strBuf2.IsEmpty() ) {
		// 計算順序の関係で１行にできない

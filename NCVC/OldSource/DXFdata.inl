...
#if !defined(___DXFDATA_INL___)
#define ___DXFDATA_INL___
...
{
	m_rcMax.left	= m_ct.x - m_r;
	m_rcMax.right	= m_ct.x + m_r;
	m_rcMax.top		= m_ct.y - m_r;
	m_rcMax.bottom	= m_ct.y + m_r;
#ifdef _DEBUG
...
inline BOOL CDXFcircle::IsMatchPoint(const CPointD& pt)
{
	...
	for ( int i=0; i<GetPointNumber(); i++ ) {
		if ( m_ptMake[i] == pt ) {
			m_nArrayExt = i & 0xfe;		// 0 or 2 (‰ºˆÊ1ËŞ¯ÄÏ½¸)
			// Šï”(180, 270)‚Ì‚¾‚¯“ü‚ê‘Ö‚¦
			if ( i & 0x01 )
				SwapPt( m_nArrayExt );
			return TRUE;
...
//////////////////////////////////////////////////////////////////////
inline void CDXFarc::ChangeRsign(void)
{
...
{
	SetEllipseArgv_Circle(lpBlock, lpArgv, m_sqDraw, m_eqDraw, m_bRoundBak);
}
...
inline int CDXFpolyline::GetVertexCount(void) const
{

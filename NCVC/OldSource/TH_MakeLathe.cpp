...
//static	BOOL	CreateInRoughPass(float);		// 内径荒加工ﾃﾞｰﾀの生成
//static	BOOL	CreateOutRoughPass(void);		// 外径荒加工ﾃﾞｰﾀの生成
static	BOOL	CreateRoughPass(int);			// 荒加工ﾃﾞｰﾀの生成
...
/*
	if ( !g_obLineTemp[0].IsEmpty() ) {
		if ( !CreateInRoughPass(dHole) )
			return FALSE;
	}
	if ( !g_obLineTemp[1].IsEmpty() ) {
		if ( !CreateOutRoughPass() )
			return FALSE;
	}
*/
	...
	for ( i=0; i<nLayerLoop && IsThread(); i++ ) {
		...
		for ( j=0; j<nLoop && IsThread(); j++ ) {
			...
			if ( pShape->ChangeCreate_MapToChain() ) {
				...
				dbg.printf("  ShapeNo.%d ChainCnt=%d", i, pChain->GetCount());
				PLIST_FOREACH(pDataDbg, pChain)
...
/*
BOOL CreateInRoughPass(float dHole)
{
	return IsThread();
}
*/
//BOOL CreateOutRoughPass(void)
...
BOOL MakeOutsideCode(const CPointF& ptMax)
{
	...
	if ( nLoop > 0 ) {
		...
		if ( pt.x < CNCMakeLathe::ms_xyz[NCA_Z] ) {
			...
			// 工具初期位置が端面の左側
			// →Z軸移動後、X軸移動
			pNCD = new CNCMakeLathe(ZXMOVE, pt, GetDbl(MKLA_DBL_O_FEED));
		}
		ASSERT(pNCD);
		g_obMakeData.Add(pNCD);
		// 先頭ﾃﾞｰﾀの切削ﾊﾟｽ
...
void MoveOutsideCode(const CDXFdata* pData, const CPointF& ptMax, const CPointF& ptPull)
{
	...
	if ( CNCMakeLathe::ms_xyz[NCA_Z] <= pt.x ) {
		...
		pt.y = ptMax.y;	// Z軸はｵﾌﾞｼﾞｪｸﾄの開始位置, X軸は外径
	pt.y += ptPull.y;					// 引き代
	pNCD = new CNCMakeLathe(XZMOVE, pt, GetDbl(MKLA_DBL_O_FEEDX));
	ASSERT(pNCD);
	g_obMakeData.Add(pNCD);

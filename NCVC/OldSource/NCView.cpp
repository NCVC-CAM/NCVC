...
BEGIN_MESSAGE_MAP(CNCView, CView)
	...
	//}}AFX_MSG_MAP
	// ﾍﾟｰｼﾞ切替ｲﾍﾞﾝﾄ
	ON_MESSAGE (WM_USERACTIVATEPAGE, &CNCView::OnUserActivatePage)
	// 各ﾋﾞｭｰへのﾌｨｯﾄﾒｯｾｰｼﾞ
	ON_MESSAGE (WM_USERVIEWFITMSG, &CNCView::OnUserViewFitMsg)
	// ﾒﾆｭｰｺﾏﾝﾄﾞ
	ON_UPDATE_COMMAND_UI(ID_EDIT_COPY, &CNCView::OnUpdateEditCopy)
	ON_UPDATE_COMMAND_UI_RANGE(ID_VIEW_UP,  ID_VIEW_RT,  &CNCView::OnUpdateMoveKey)
	ON_UPDATE_COMMAND_UI_RANGE(ID_VIEW_RUP, ID_VIEW_RRT, &CNCView::OnUpdateRoundKey)
	ON_COMMAND(ID_EDIT_COPY, &CNCView::OnEditCopy)
	ON_COMMAND_RANGE(ID_VIEW_UP,     ID_VIEW_RT,    &CNCView::OnMoveKey)
	ON_COMMAND_RANGE(ID_VIEW_BEFORE, ID_VIEW_LENSN, &CNCView::OnLensKey)
END_MESSAGE_MAP()
...
void CNCView::OnDraw(CDC* pDC)
{
	...
	for ( i=GetDocument()->GetTraceStart(); i<nDraw; i++ ) {
		...
		if ( pData->GetGtype() == G_TYPE )
			(pData->*m_pfnDrawProc)(pDC, FALSE);	// pData->Draw(pDC, FALSE);
	}
...
void CNCView::OnUpdateMoveKey(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(TRUE);
}

void CNCView::OnUpdateRoundKey(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(FALSE);
}
...
{
#ifdef _DEBUG_FILEOPEN
	g_dbg.printf("CNCView::OnCreate() Start");
#endif

...
BEGIN_MESSAGE_MAP(CNCViewYZ, CView)
	...
	//}}AFX_MSG_MAP
	// ﾍﾟｰｼﾞ切替ｲﾍﾞﾝﾄ
	ON_MESSAGE (WM_USERACTIVATEPAGE, &CNCViewYZ::OnUserActivatePage)
	// 各ﾋﾞｭｰへのﾌｨｯﾄﾒｯｾｰｼﾞ
	ON_MESSAGE (WM_USERVIEWFITMSG, &CNCViewYZ::OnUserViewFitMsg)
	// ﾒﾆｭｰｺﾏﾝﾄﾞ
	ON_UPDATE_COMMAND_UI(ID_EDIT_COPY, &CNCViewYZ::OnUpdateEditCopy)
	ON_UPDATE_COMMAND_UI_RANGE(ID_VIEW_UP,  ID_VIEW_RT,  &CNCViewYZ::OnUpdateMoveKey)
	ON_UPDATE_COMMAND_UI_RANGE(ID_VIEW_RUP, ID_VIEW_RRT, &CNCViewYZ::OnUpdateRoundKey)
	ON_COMMAND(ID_EDIT_COPY, &CNCViewYZ::OnEditCopy)
	ON_COMMAND_RANGE(ID_VIEW_UP,     ID_VIEW_RT,    &CNCViewYZ::OnMoveKey)
	ON_COMMAND_RANGE(ID_VIEW_BEFORE, ID_VIEW_LENSN, &CNCViewYZ::OnLensKey)
END_MESSAGE_MAP()
...
{
#ifdef _DEBUG_CMDMSG
	g_dbg.printf("CNCViewYZ::OnCmdMsg()");
#endif
...
void CNCViewYZ::OnUpdateMoveKey(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(TRUE);
}

void CNCViewYZ::OnUpdateRoundKey(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(FALSE);
}

...
//	ﾏﾙﾁｽﾚｯﾄﾞで実行するNCﾌﾟﾛｸﾞﾗﾑ量
static	const int		MAXNCPROG = 4;
#else
static	const int		MAXNCPROG = 10000;
#endif
...
// 切削ﾊﾟｽ生成ｽﾚｯﾄﾞ用
typedef	struct	tagCREATEVERTEXPARAM {
	CNCDoc*	pDoc;
	vector<VECPOINT3D>*	pVertexStrip;
	vector<VECPOINT3D>*	pVertexFan;
	int		cs, ce;
	BOOL	bResult;
} CREATEVERTEXPARAM, *LPCREATEVERTEXPARAM;
...
static	DWORD WINAPI CreateBottomFaceVertexThread(LPVOID);
static	UINT		 DeleteVertexThread(LPVOID);
static	DWORD WINAPI CreateElement_WrkThreadTop(LPVOID);
static	DWORD WINAPI CreateElement_WrkThreadBtm(LPVOID);
...
//	if ( g_nProcesser>1 && nLoop>MAXNCPROG ) {
//		CreateBottomVertex();
//	}
//	else {
		for ( i=0; i<nLoop; ++i )
...
BOOL CNCViewGL::CreateBottomVertex(void)
{
#ifdef _DEBUG
	CMagaDbg	dbg("CreateBottomVertex()\nStart");
#endif

	HANDLE*					phThread;
	LPCREATEVERTEXPARAM		pParam;

	phThread = new HANDLE[g_nProcesser];
	pParam   = new CREATEVERTEXPARAM[g_nProcesser];

	int		nMax = GetDocument()->GetNCsize(),
			i, n = nMax / g_nProcesser;	// ｽﾚｯﾄﾞごとの処理数

	pParam[0].cs = 0;
	for ( i=0; i<g_nProcesser-1; i++ ) {
		pParam[i].pDoc = GetDocument();
		pParam[i].pVertexStrip = new vector<VECPOINT3D>;
		pParam[i].pVertexFan   = new vector<VECPOINT3D>;
		pParam[i].pVertexStrip->reserve(n);	// 増加の目安
		pParam[i].pVertexFan->reserve(n);
		pParam[i].ce = pParam[i+1].cs = min(pParam[i].cs + n, nMax);
		pParam[i].bResult = TRUE;
		phThread[i] = ::CreateThread(NULL, 0, CreateBottomFaceVertexThread, &pParam[i], 0, NULL);
	}
	pParam[i].pDoc = GetDocument();
	pParam[i].pVertexStrip = new vector<VECPOINT3D>;
	pParam[i].pVertexFan   = new vector<VECPOINT3D>;
	pParam[i].pVertexStrip->reserve(n);
	pParam[i].pVertexFan->reserve(n);
	pParam[i].ce = nMax;
	pParam[i].bResult = TRUE;
	phThread[i] = ::CreateThread(NULL, 0, CreateBottomFaceVertexThread, &pParam[i], 0, NULL);
	::WaitForMultipleObjects(g_nProcesser, phThread, TRUE, INFINITE);
	for ( i=0; i<g_nProcesser; i++ )
		::CloseHandle(phThread[i]);
	for ( i=0; i<g_nProcesser; i++ ) {
		if ( !pParam[i].bResult ) {
			for ( i=0; i<g_nProcesser; i++ ) {
				delete pParam[i].pVertexStrip;
				delete pParam[i].pVertexFan;
			}
			delete pParam;
			delete phThread;
			return FALSE;
	...
	for ( i=0; i<g_nProcesser; i++ ) {
		nSize = pParam[i].pVertexStrip->size();
		for ( ii=0; ii<nSize; ++ii ) {
			VECPOINT3D&	vStrip = pParam[i].pVertexStrip->operator[](ii);
			::glVertexPointer(NCXYZ, GL_DOUBLE, 0, &vStrip[0]);
			if ( ::glGetError() != GL_NO_ERROR ) {	// GL_OUT_OF_MEMORY
				AfxMessageBox(IDS_ERR_OUTOFVRAM, MB_OK|MB_ICONEXCLAMATION);
				break;
			}
			::glDrawArrays(GL_TRIANGLE_STRIP, 0, vStrip.size());
		}
		nSize = pParam[i].pVertexFan->size();
		for ( ii=0; ii<nSize; ++ii ) {
			VECPOINT3D&	vFan = pParam[i].pVertexFan->operator[](ii);
			::glVertexPointer(NCXYZ, GL_DOUBLE, 0, &vFan[0]);
			if ( ::glGetError() != GL_NO_ERROR ) {
				AfxMessageBox(IDS_ERR_OUTOFVRAM, MB_OK|MB_ICONEXCLAMATION);
				break;
			}
			::glDrawArrays(GL_TRIANGLE_FAN, 0, vFan.size());
		}
	}
	::glDisableClientState(GL_VERTEX_ARRAY);

	// 後片付け
	AfxBeginThread(DeleteVertexThread, pParam, THREAD_PRIORITY_IDLE);
	delete phThread;

	return TRUE;
}

DWORD WINAPI CreateBottomFaceVertexThread(LPVOID pVoid)
{
	LPCREATEVERTEXPARAM pParam = reinterpret_cast<LPCREATEVERTEXPARAM>(pVoid);

#ifdef _DEBUG
	CMagaDbg	dbg("CreateBottomFaceVertexThread()\nStart", DBG_BLUE);
	DWORD	t1 = ::timeGetTime();
#endif
	...
	try {
		for ( i=pParam->cs; i<pParam->ce; ++i ) {
			pParam->pDoc->GetNCdata(i)->AddBottomVertex(pParam->pVertexStrip, pParam->pVertexFan);
		}
	}
...
UINT DeleteVertexThread(LPVOID pVoid)
{
	LPCREATEVERTEXPARAM pParam = reinterpret_cast<LPCREATEVERTEXPARAM>(pVoid);

#ifdef _DEBUG
	CMagaDbg	dbg("DeleteVertexThread()\nStart", DBG_BLUE);
	DWORD	t1 = ::timeGetTime();
#endif
...
BOOL CNCViewGL::CreateVBOMill(const GLfloat* pfXYZ, GLfloat* pfNOR)
{
	...
	HANDLE				hThread[4];
	CREATEELEMENTPARAM	param[4];

	// 頂点配列ｲﾝﾃﾞｯｸｽ(可変長２次元配列)
	vector<CVElement>	vElementWrkTop,
						vElementWrkBtm,
						vElementCut1,
						vElementCut2;
	vElementWrkTop.reserve(m_icy+1);
	vElementWrkBtm.reserve(m_icy+1);

	if ( g_nProcesser > 1 ) {
		// ﾏﾙﾁCPUの場合、切削面を2ｽﾚｯﾄﾞで半分ずつ走査
		param[0].cs = 0;			param[0].ce = m_icy / 2;
		param[1].cs = param[0].ce;	param[1].ce = m_icy - 1;
		vElementCut1.reserve(m_icy/2+1);
		vElementCut2.reserve(m_icy/2+1);
	}
	else {
		// ｼﾝｸﾞﾙCPUの場合は1ｽﾚｯﾄﾞで切削面を走査
		param[0].cs = 0;			param[0].ce = m_icy - 1;
		param[1].cs = param[1].ce = param[0].ce;
		vElementCut1.reserve(m_icy+1);
	}

	// ﾜｰｸ(上面/底面)と切削面*2を４つのｽﾚｯﾄﾞで処理
	...
	param[0].pElement = &vElementCut1;
	param[1].pElement = &vElementCut2;
	param[2].pElement = &vElementWrkTop;
	param[3].pElement = &vElementWrkBtm;
	hThread[0] = ::CreateThread(NULL, 0, CreateElement_CutThread,    &param[0], 0, NULL);
	hThread[1] = ::CreateThread(NULL, 0, CreateElement_CutThread,    &param[1], 0, NULL);
	hThread[2] = ::CreateThread(NULL, 0, CreateElement_WrkThreadTop, &param[2], 0, NULL);
	hThread[3] = ::CreateThread(NULL, 0, CreateElement_WrkThreadBtm, &param[3], 0, NULL);
	::WaitForMultipleObjects(SIZEOF(hThread), hThread, TRUE, INFINITE);
	::CloseHandle(hThread[0]);
	::CloseHandle(hThread[1]);
	::CloseHandle(hThread[2]);
	::CloseHandle(hThread[3]);
	if ( !param[0].bResult || !param[1].bResult || !param[2].bResult || !param[3].bResult )
		return FALSE;
	...
	try {
		size_t	ii, jj,		// 警告防止
				nElement,
				nCutSize1   = vElementCut1.size(),
				nCutSize2   = vElementCut2.size(),
				nWrkSizeTop = vElementWrkTop.size(),
				nWrkSizeBtm = vElementWrkBtm.size(),
				nWrkSize = nWrkSizeTop + nWrkSizeBtm,
				nCutSize = nCutSize1   + nCutSize2;
...
#endif
		// ﾜｰｸ矩形(上面)用
		for ( ii=jj=0; ii<nWrkSizeTop; ++ii, ++jj ) {
			nElement = vElementWrkTop[ii].size();
			::glBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, m_pGenBuf[jj]);
			::glBufferDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB,
				nElement*sizeof(GLuint), &vElementWrkTop[ii][0],
				GL_STATIC_DRAW_ARB);
			if ( ::glGetError() != GL_NO_ERROR ) {
		...
		// ﾜｰｸ矩形(底面)用
		for ( ii=0; ii<nWrkSizeBtm; ++ii, ++jj ) {
			nElement = vElementWrkBtm[ii].size();
			::glBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, m_pGenBuf[jj]);
			::glBufferDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB,
				nElement*sizeof(GLuint), &vElementWrkBtm[ii][0],
				GL_STATIC_DRAW_ARB);
			if ( ::glGetError() != GL_NO_ERROR ) {
				::glBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, 0);
				ClearVBO();
		...
		// 切削面用
		for ( ii=0; ii<nCutSize1; ++ii, ++jj ) {
			nElement = vElementCut1[ii].size();
			::glBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, m_pGenBuf[jj]);
			::glBufferDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB,
				nElement*sizeof(GLuint), &vElementCut1[ii][0],
				GL_STATIC_DRAW_ARB);
			if ( ::glGetError() != GL_NO_ERROR ) {
				::glBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, 0);
				ClearVBO();
		...
		for ( ii=0; ii<nCutSize2; ++ii, ++jj ) {
			nElement = vElementCut2[ii].size();
			::glBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, m_pGenBuf[jj]);
			::glBufferDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB,
				nElement*sizeof(GLuint), &vElementCut2[ii][0],
				GL_STATIC_DRAW_ARB);
			if ( ::glGetError() != GL_NO_ERROR ) {
				::glBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, 0);
				ClearVBO();
...
DWORD WINAPI CreateElement_WrkThreadTop(LPVOID pVoid)
{
#ifdef _DEBUG
	CMagaDbg	dbg("CreateElement_WrkThreadTop()\nStart", DBG_BLUE);
	DWORD	t1 = ::timeGetTime();
#endif
...
DWORD WINAPI CreateElement_WrkThreadBtm(LPVOID pVoid)
{
#ifdef _DEBUG
	CMagaDbg	dbg("CreateElement_WrkThreadBtm()\nStart", DBG_BLUE);
	DWORD	t1 = ::timeGetTime();
#endif
...
DWORD WINAPI CreateElement_CutThread(LPVOID pVoid)
{
	...
	try {
		// 切削面（上面・側面兼用）
		// ○：切削面，△：貫通，×：ﾜｰｸ上面
		for ( j=pParam->cs; j<pParam->ce; ++j ) {	// ﾙｰﾌﾟ条件は親でｾｯﾄ

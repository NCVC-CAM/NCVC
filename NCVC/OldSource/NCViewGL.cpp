...
void CNCViewGL::CreateBoxel(void)
{
	...
	::glClearDepth(0.0);		// 遠い方を先に優先させるための値
	...
	for ( i=0; i<GetDocument()->GetNCsize(); i++ )
		GetDocument()->GetNCdata(i)->DrawBottomFace(z);
	// ﾃﾞﾌﾟｽ値の取得
	ClipDepth();
...
void CNCViewGL::ClipDepth(void)
{
	...
	GLfloat		fx, fy, fz;
	GLfloat*	pfDepth;		// ﾃﾞﾌﾟｽ値取得配列一時領域
...
//	::glReadPixels(0, 0, m_icx, m_icy,
	...
	for ( j=0; j<m_icy; j++ ) {
		for ( i=0; i<m_icx; i++ ) {
			n = j*m_icy+i;
			if ( pfDepth[n] == 0.0 ) {	// 初期値なら変換必要なし
				fx = fy = fz = HUGE_VAL;
			}
			else {
//				::gluUnProject(i, j, pfDepth[n],
			...
			m_vfXYZ.push_back(fx);
			m_vfXYZ.push_back(fy);
			m_vfXYZ.push_back(fz);
		}
	}
	...
	for ( j=0; j<m_icy; j++ ) {
		...
		for ( i=0; i<m_icx; i++ ) {
			...
			if ( pfDepth[n] != 0.0 ) {
				...
				s += r;
				r.Format(IDS_MAKENCD_FORMAT, m_vfXYZ[n*NCXYZ+NCA_X]);
				sx += r;
				r.Format(IDS_MAKENCD_FORMAT, m_vfXYZ[n*NCXYZ+NCA_Y]);
				sy += r;
				r.Format(IDS_MAKENCD_FORMAT, m_vfXYZ[n*NCXYZ+NCA_Z]);
				sz += r;
			}
		}
		if ( !s.IsEmpty() ) {
...
//	double	wx, wy, wz,
//			znr = pjMatrix[14]/(pjMatrix[10]-1.0),
...
*/
	...
	for ( j=0; j<m_icy-1; j++ ) {
		for ( i=0; i<m_icx-1; i++ ) {
			n1 = ( j   *m_icy+i)   * NCXYZ;
			if ( m_vfXYZ[n1+NCA_X]==HUGE_VAL || m_rcDraw.low>=m_vfXYZ[n1+NCA_Z] )
				continue;
			n2 = ( j   *m_icy+i+1) * NCXYZ;
			n3 = ((j+1)*m_icy+i+1) * NCXYZ;
			n4 = ((j+1)*m_icy+i)   * NCXYZ;
			::glVertex3d(m_vfXYZ[n1+NCA_X], m_vfXYZ[n1+NCA_Y], m_vfXYZ[n1+NCA_Z]);
			::glVertex3d(m_vfXYZ[n2+NCA_X], m_vfXYZ[n2+NCA_Y], m_vfXYZ[n2+NCA_Z]);
			::glVertex3d(m_vfXYZ[n3+NCA_X], m_vfXYZ[n3+NCA_Y], m_vfXYZ[n3+NCA_Z]);
			::glVertex3d(m_vfXYZ[n4+NCA_X], m_vfXYZ[n4+NCA_Y], m_vfXYZ[n4+NCA_Z]);
		}
	}
	::glEnd();
...
/*
	...
	// 切削底面描画(ｽﾃﾝｼﾙﾃｽﾄOFF)
	// ｽﾃﾝｼﾙﾋﾞｯﾄが1の所(側面が描画されていない場所)だけ描画
	::glEnable (GL_LIGHT2);
	::glEnable (GL_LIGHT3);
	::glDisable(GL_LIGHT4); 
	::glDisable(GL_LIGHT5);

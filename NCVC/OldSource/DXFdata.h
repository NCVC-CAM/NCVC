...
#if !defined(AFX_DXFDATA_H__98866C20_9823_11D3_B0D5_004005691B12__INCLUDED_)
#define AFX_DXFDATA_H__98866C20_9823_11D3_B0D5_004005691B12__INCLUDED_
...
//	BOOL	GetOutlineArraw_Line(const CPointD[], const CPointD&, double, CPointD&, CPointD[][3]) const;
//	BOOL	GetOutlineArraw_Circle(LPDXFAARGV, const CPointD&, double, CPointD&, CPointD[][3]) const;
...
//	virtual	BOOL	GetOutlineArraw(const CPointD&, double, CPointD&, CPointD[][3]) const = 0;
	virtual	BOOL	SetDirectionFixed(const CPointD&, const CPointD&) = 0;
...
//	virtual	BOOL	GetOutlineArraw(const CPointD&, double, CPointD&, CPointD[][3]) const;
	virtual	BOOL	SetDirectionFixed(const CPointD&, const CPointD&);
...
protected:
	...
	BOOL	m_bRound,		// G2(FALSE)/G3(TRUE) DXFの基本は G3(反時計回り)
			m_bRoundBak;	// 生成中に向きが変わる可能性があるのでﾊﾞｯｸｱｯﾌﾟ
...
{
	void	ChangeRsign(void);

protected:
	double		m_sq, m_eq,		// 開始・終了角度(ﾗｼﾞｱﾝ単位)
				m_sqDraw, m_eqDraw,	m_rDraw;	// 描画用(Swapしない)

	void	SetMaxRect(CPointD* = NULL, CPointD* = NULL);	// 矩形計算(楕円用引数)
...
#endif // !defined(AFX_DXFDATA_H__98866C20_9823_11D3_B0D5_004005691B12__INCLUDED_)

...
BOOL CMakeNCDlgEx3::OnInitDialog() 
{
	...
	for ( i=0, nCnt=0; i<nLoop; i++ ) {
		...
		if ( pLayer->IsCutType() ) {
			...
			m_ctLayerList.SetCheck(nCnt++, pLayer->m_bCutTarget);
		}
	}
	// ﾚｲﾔﾘｽﾄの並べ替え
	m_ctLayerList.SortItems(CompareFunc, nSortLayer);
	// ﾍｯﾀﾞｰﾏｰｸ挿入
	SetHeaderMark(nSortLayer);		// m_nSortLayerへ代入
...
BOOL CMakeNCDlgEx3::OnWizardFinish()
{
	...
	for ( i=0; i<nLoop; i++ ) {
		...
		pLayer = reinterpret_cast<CLayerData *>(m_ctLayerList.GetItemData(i));
		// 出力順の設定
		pLayer->SetLayerListNo(pLayer->IsPartOut() ? -1 : i);
		// 個別出力の有無
		if ( pLayer->IsPartOut() ) {
...
BOOL CMakeNCDlgEx3::OnKillActive() 
{
	...
	for ( int i=0; i<m_ctLayerList.GetItemCount(); i++ ) {
		pLayer = reinterpret_cast<CLayerData *>(m_ctLayerList.GetItemData(i));
		if ( (pLayer->m_bCutTarget = m_ctLayerList.GetCheck(i)) )	// 代入兼用
			bCutCheck = TRUE;
	}
	// 切削対象が１つもない
	if ( !bCutCheck ) {
...
void CMakeNCDlgEx3::OnColumnClickLayerList(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if ( abs(m_nSortLayer) == nNewColumn )
		nNewColumn = -m_nSortLayer;	// 昇順・降順の切り替え
	// 並べ替え

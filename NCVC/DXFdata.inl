...
{
	return IsMakeMatchPoint( pData->GetEndMakePoint() );	// virtualw’è
}
...
{
	return ( m_dwPolyFlags & DXFPOLY_INTERSEC );	// ©•ª©g‚ÉŒğ“_‚ ‚è
}

inline BOOL CDXFpolyline::IsStartEqEnd(void) const
{
	return ( m_dwPolyFlags & DXFPOLY_CLOSED );	// •ÂÙ°Ìß
}
...
{
	return HUGE_VAL;	// IsMakeMatchPoint() ˆÈŠO‚Í‹ßÚ”»’f‚Ì•K—v‚È‚µ
}

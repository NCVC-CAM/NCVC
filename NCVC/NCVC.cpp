...
extern	int		g_nLastPage_MachineSetup = 0;	// 工作機械ｵﾌﾟｼｮﾝ
...
BEGIN_MESSAGE_MAP(CNCVCApp, CWinAppEx)
	...
	ON_COMMAND_RANGE(ID_HELP_USING2, ID_HELP_USING7, &CNCVCApp::OnHelpUsing)
...
BOOL CNCVCApp::InitInstance()
{
	...
	m_pDocTemplate[TYPE_3DM] = new CNCVCDocTemplate(	// 3DMﾄﾞｷｭﾒﾝﾄ
		IDR_3DMTYPE,	// この時点で複数の拡張子登録は不可
...
{
	static	LPCTSTR	lpszAddin = "アドイン(&A)";
...
void CNCVCApp::OnOptionMC() 
{
	...
	if ( ps.DoModal() != IDOK ) {
		if ( !strFileName.IsEmpty() )
			m_pOptMC->ReadMachineOption(strFileName, FALSE);	// 設定前のﾌｧｲﾙで読み直し
		return;
	}
